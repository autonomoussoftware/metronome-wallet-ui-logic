 ...
 *
 * @param {Object} config - Params required for the estimate
 * @param {string} config.MET_TOKEN_ADDR - The Metronome token contract address
 * @param {string} config.CONVERTER_ADDR - The Converter contract address
 * @param {string} str A message string.
 */
export function messageParser(config, str) {
  const replacements = [
